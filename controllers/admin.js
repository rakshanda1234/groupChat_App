...
exports.addUser = async (req, res, next) => {
  try {
    ...
    if (!userToBeAdded) {
      return res
        .status(400)
        .json({ message: "member to be added is not registered" });
    }
...
exports.makeAdmin = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "member to be added is no more registered" });
    }
    const verifiedAdmin = await GroupUser.findOne({
      where: {
        [Op.and]: [
    ...
    let memberToBeUpdated = await GroupUser.findOne({
      where: {
        [Op.and]: [{ userId: userIdToBeMadeAdmin }, { groupId: groupId }],
      },
    });
...
exports.removeUserFromGroup = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "user to be removed is no more registered" });
    }
    const verifiedAdmin = await GroupUser.findOne({
      where: {
        [Op.and]: [
    ...
    let memberToBeRemoved = await GroupUser.findOne({
      where: {
        [Op.and]: [{ userId: userIdToBeRemoved }, { groupId: groupId }],
      },
    });
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message: "member to be removed as admin is no more registered",
      });
    }
    const verifiedAdmin = await GroupUser.findOne({
      where: {
    ...
    let memberToBeUpdated = await GroupUser.findOne({
      where: {
        [Op.and]: [{ userId: userIdToBeUpdated }, { groupId: groupId }],
      },
    });
